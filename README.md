...
| `GitLab__BaseAddress` | `https://gitlab.com/api/v4/` | Which GitLab instance to talk to. Point this at a self-hosted install by domain (`https://gitlab.example.com/api/v4/`), a path-prefixed self-hosted install (`https://example.com/gitlab/api/v4/`), or a non-standard port (`https://gitlab.internal:8443/api/v4/`) — all three are covered by `tests/GitlabMCP.Tests/GraphQL/GitLabGraphQlEndpointTests.cs`. **Must end in `api/v4/`**, not just a trailing slash — a bare instance root (e.g. `https://gitlab.example.com/`) fails at startup with a clear message rather than breaking epic/work-item tools later. |
