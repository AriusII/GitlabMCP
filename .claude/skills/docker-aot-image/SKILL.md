...
| `GitLab:BaseAddress` | `GitLab__BaseAddress` | `GitLabClientOptions.BaseAddress`; absent → `https://gitlab.com/api/v4/`; must be absolute and end in `/`. Self-hosted, a custom domain, a path-prefixed install, and a non-standard port all work — `GitLabGraphQlEndpointTests` pins this down | `gitlab-client-navigation` §Registration and options | **Live** |
