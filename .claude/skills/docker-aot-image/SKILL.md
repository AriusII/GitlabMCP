...
| Same `NETSDK1064` **in GitHub Actions specifically**, on a `docker/build-push-action` step using `cache-from`/`cache-to: type=gha`, when it built fine locally | The exact same independent-lifetimes bug, one layer up: `type=gha` caches *layers*, not `RUN --mount=type=cache` contents. On a fresh ephemeral runner, a gha cache-hit on the restore layer (its execution — and the mount-fill it would have done — gets skipped entirely) followed by a live publish run (because a later `COPY src/` changed) leaves publish's mount empty. Reproduced on this repo 2026-09-09 (DEC-035). Fix: drop `cache-from`/`cache-to` from that build step entirely — restore and publish must always execute together, in the same live BuildKit session, never one imported and the other live. |
